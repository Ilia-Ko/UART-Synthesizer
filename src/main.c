...
// GPIO on AHB1
// DAC and TIM2, TIM5 on APB1
// USART1 on APB2

#define HCLK_FREQ (48*1000*1000UL)

// 1 Hz LED  on |PG13| (active HIGH)
// UART LED  on |PG14| (active HIGH)
// DAC1.OUT2 on |PA5|
// ADC1.INP6 on |PA6|
// USART1.TX on |PB6|
// USART1.RX on |PB7|
...
// --- --- --- UART defines --- --- ---
#define BAUD_RATE (1200*1000UL)     // Signalling rate for UART to maintain 768 kbps audio transfer
#define RX_BUF_SIZE 1024            // RAM buffer length to receive commands
#define RX_BUF_HALF (RX_BUF_SIZE/2) // Half of the RX buffer
#define TX_BUF_SIZE 256             // RAM buffer length to compose messages
#define CMD_BUF_SIZE 64             // Command buffer
...
volatile uint32_t rxPos;    // Current position inside RX buffer, used by UART
volatile uint32_t sPos;     // Current sample position in RX buffer, used by DAC
bool isOverflow;            // Whether buffer was overflown
volatile bool isPending;    // Whether a command is pending
volatile enum REGIME regime;// Current operation regime
...
#define FILT_DEPTH 6        // Maximal power of autoregressive filter
#define FILT_MEM 1024       // Sample memory depth for autoregressive equalizer
...
volatile uint32_t iirPos;       // Cyclic position in the long-term or short-term buffers
...
void FireOneShot(uint32_t durMS) {      // Issue a 'durMS' milliseconds delay
    ...
    LL_TIM_SetAutoReload(TIM3, durMS);  // Program TIM3 for 'durMS' delay
...
uint32_t EntropyADC(void) { // Get next random from floating ADC
    ...
    uint32_t rnd = LL_ADC_REG_ReadConversionData12(ADC1);
    LL_ADC_REG_StartConversionSWStart(ADC1);    // Launch a new conversion for future
    return rnd;
}
...
bool ExecCommand(char* cmd, uint32_t cmdLen) {          // Execute user command and return success flag
    ...
    if (0 == strncmp(cmd, CMD_PING, sizeof CMD_PING)) {             // Compare incl. 0
        ...
        case PAUSING:
            ...
            break;
        case ONESHOT:
            uint32_t durMS = LL_TIM_GetAutoReload(TIM3);
            uint32_t remMS = durMS - LL_TIM_GetCounter(TIM3);
            if (isNoise) {
        ...
        case IMPROV:
            ...
            break;
        case STREAM:
            pos += snprintf(txBuf+pos, TX_BUF_SIZE-pos, "streaming 16(12)-bit mono 48 kHz (768 kbps)");
            break;
        default:
...
void SetupHSE(void) {       // Switch to HSE clock
    ...
    // Increase the count of FLASH wait states (to accomodate its latency)
    FLASH->ACR |= FLASH_ACR_LATENCY_1WS;    // 1 WS for 30 < HCLK < 60 MHz
    if ((FLASH->ACR & FLASH_ACR_LATENCY_Msk) != FLASH_ACR_LATENCY_1WS) while (TRUE);

    // Configure PLL so that SYSCLK frequency were 96 MHz and USB,RNG frequency were 48 MHz
    // 0. HSE @ 8 MHz = PLL_IN
    // 1. PLL_IN / PLLM = VCO_IN = 2 MHz (thus PLLM = 4)            Must be: 1 MHz <= VCO_IN <= 2 MHz
    // 2. VCO_IN * PLLN = VCO_OUT = 384 MHz (thus PLLN = 192)       Must be: 192 MHz <= VCO_OUT <= 432 MHz
    // 3. VCO_OUT / PLLP = PLL_OUT = 96 MHz (thus PLLP = 4)         Must be: PLL_OUT <= 180 MHz
    // 4. VCO_OUT / PLLQ = USB_RNG = 48 MHz (thus PLLQ = 8)         Must be: USB_RNG = 48 MHz
    uint32_t pllM = 4U << RCC_PLLCFGR_PLLM_Pos;     // This factor is common for PLL, PLLI2S, PLLSAI
    uint32_t pllN = 192U << RCC_PLLCFGR_PLLN_Pos;
    uint32_t pllP = 0b01U << RCC_PLLCFGR_PLLP_Pos;
    uint32_t pllQ = 8U << RCC_PLLCFGR_PLLQ_Pos;
    LL_RCC_PLL_Disable();
    RCC->PLLCFGR = RCC_PLLCFGR_PLLSRC_HSE | pllM | pllN | pllP | pllQ;  // Configure PLL

    // Start the PLL
    LL_RCC_PLL_Enable();
    while (!LL_RCC_PLL_IsReady());  // Wait until PLL becomes ready

    // WARNING: if AHB > 30 MHz FLASH wait cycles should be adjusted
    // WARNING: if AHB > 120 MHz PWR overdrive mode should be configured

    // Configure bus prescalers: AHB 48 MHz, APB1 24 MHz, APB2 48 MHz
    LL_RCC_SetAHBPrescaler(LL_RCC_SYSCLK_DIV_2);    // AHB freq must be <= 180 MHz
    LL_RCC_SetAPB1Prescaler(LL_RCC_APB1_DIV_2);     // APB1 freq must be <= 45 MHz
    LL_RCC_SetAPB2Prescaler(LL_RCC_APB2_DIV_1);     // APB2 freq must be <= 90 MHz
    LL_RCC_SetTIMPrescaler(LL_RCC_TIM_PRESCALER_FOUR_TIMES);    // This makes TIM clocks run AHB frequency
    LL_RCC_SetSysClkSource(LL_RCC_SYS_CLKSOURCE_PLL);
}
void SetupDAC(void) {       // Configure DAC on PA5 analog pin
    // Setup GPIO PA5 analog pin
    LL_AHB1_GRP1_EnableClock(LL_AHB1_GRP1_PERIPH_GPIOA);
    LL_GPIO_SetPinMode(GPIOA, LL_GPIO_PIN_5, LL_GPIO_MODE_ANALOG);

    // Configure DAC channel 2 to be triggered by software
    LL_APB1_GRP1_EnableClock(LL_APB1_GRP1_PERIPH_DAC1);
    LL_DAC_EnableTrigger(DAC, LL_DAC_CHANNEL_2);
    LL_DAC_SetTriggerSource(DAC, LL_DAC_CHANNEL_2, LL_DAC_TRIG_SOFTWARE);

    // Reset buffer
    LL_DAC_ConvertData12RightAligned(DAC, LL_DAC_CHANNEL_2, 0x000);
    LL_DAC_Enable(DAC, LL_DAC_CHANNEL_2);
}
void SetupRandoms(void) {   // Configure RNG and ADC1 channel 6 to measure floating analog PA6 pin
    // Configure GPIO analog PA6 pin
    LL_AHB1_GRP1_EnableClock(LL_AHB1_GRP1_PERIPH_GPIOA);
...
void SetupBaseTimer(uint32_t timFreq) { // Configure TIM2 to generate IRQs at 'BASE_FREQ' frequency
    ...
    NVIC_SetPriority(TIM2_IRQn, 35);    // Ensure default priority
...
void SetupMSTimer(uint32_t timFreq) {   // Configure TIM3 in one-shot mode
    ...
    // Configure frequency
    uint32_t psc = timFreq / 1000 - 1;
    LL_TIM_SetPrescaler(TIM3, psc);     // Divide by smth -> 1 kHz
...
void SetupDebugTimer(uint32_t timFreq) {// Configure TIM5 to generate IRQs at 2 Hz frequency
    // Setup GPIO PG13 for LED
    LL_AHB1_GRP1_EnableClock(LL_AHB1_GRP1_PERIPH_GPIOG);
    LL_GPIO_SetPinMode(GPIOG, LL_GPIO_PIN_13, LL_GPIO_MODE_OUTPUT);
    LL_GPIO_ResetOutputPin(GPIOG, LL_GPIO_PIN_13);  // Turn off LED by default
...
void SetupUART(uint32_t apb2Freq) {     // Configure UART on GPIOB pins PB6 (TX) and PB7 (RX)
    // Configure GPIO PG14 as output for LED
    LL_AHB1_GRP1_EnableClock(LL_AHB1_GRP1_PERIPH_GPIOG);
    LL_GPIO_SetPinMode(GPIOG, LL_GPIO_PIN_14, LL_GPIO_MODE_OUTPUT);
    LL_GPIO_ResetOutputPin(GPIOG, LL_GPIO_PIN_14);  // Turn off LED by default

    // Configure GPIO PA0 for input from user button
    LL_AHB1_GRP1_EnableClock(LL_AHB1_GRP1_PERIPH_GPIOA);
    LL_GPIO_SetPinMode(GPIOA, LL_GPIO_PIN_0, LL_GPIO_MODE_INPUT);

    // Configure EXTI line 0 interrupt
    EXTI->IMR |= EXTI_IMR_IM0;  // Enable EXTI0 line in EXTI Interrupt Mask Register
    EXTI->RTSR |= EXTI_RTSR_TR0;// Enable rising edge interrupt trigger
    NVIC_EnableIRQ(EXTI0_IRQn); // Enable IRQ from EXTI0 line on NVIC

    // Configure GPIO pins PB6 and PB7
    LL_AHB1_GRP1_EnableClock(LL_AHB1_GRP1_PERIPH_GPIOB);    // Enable GPIOB block
    ...
    LL_USART_SetBaudRate(USART1, apb2Freq, LL_USART_OVERSAMPLING_16, BAUD_RATE);    // Set baud rate

    // Setup UART operation regime
    LL_USART_DisableIT_ERROR(USART1);   // Do not generate false interrupts with ORE
    ...
    NVIC_EnableIRQ(USART1_IRQn);        // See 'stm32f429xx.h' line 114
    NVIC_SetPriority(USART1_IRQn, 44);  // Ensure default priority
    // IRQ handler is named 'USART1_IRQHandler', see 'startup_stm32f429xx.s' line 183
    LL_USART_Enable(USART1);            // Start the operation of USART1
...
int main(void) {
    ...
    // Setup system clock
    LL_RCC_GetSystemClocksFreq(&clk);   // Get all frequencies
    while (HCLK_FREQ != clk.HCLK_Frequency);
...
void TIM5_IRQHandler(void) {    // This IRQ is triggered 2 times per second by TIM5
    ...
    LL_GPIO_TogglePin(GPIOG, LL_GPIO_PIN_13);   // Change status LED state
...
void TIM2_IRQHandler(void) {    // This IRQ is triggered 'BASE_FREQ' times per second by TIM2
    ...
    if (STREAM == regime) {
        ...
        // Get sample from RX buffer half
        sample = *((int16_t*) (rxBuf + sPos));  // Load signed 16-bit sample from RX buffer
        sample += (1U << 15);   // Convert signed 16-bit sample into unsigned 16-bit sample
        sample &= 0xFFFFU;      // Remove 17-th bit after s2u conversion
        sample >>= 4;           // Reduce 16-bit precision to 12-bit
        if (doFilter) {
            // Apply filter to streamed audio samples if needed
            float32_t s = 2.0f * (sample / (float32_t) MAX_CODE) - 1;
            s = FilterSample(s);
            // Scale streamed audio by the amplitude and clamp if needed
            s *= amp;
            if (s > +1.1f) s = +1;
            if (s < -1.1f) s = -1;
            sample = MID_CODE * (1.0f + s) - 1U;
        }
        sPos = (sPos + 2U) % RX_BUF_SIZE;   // Move to the next 16-bit sample in buffer
    } else if (IDLE == regime) {
        ...
        if (isNoise) {
            ...
            case WAVE_SIN5:
                ...
                s1 = arm_sin_f32(t * oldFreq + phi);
                s2 = arm_sin_f32(t * freq + phi);
                s1 = arm_exponent_f32(s1,5);
                s2 = arm_exponent_f32(s2,5);
                break;
            case WAVE_RECT:
    ...
    // Convert next sample
    LL_DAC_ConvertData12RightAligned(DAC, LL_DAC_CHANNEL_2, sample);
...
void USART1_IRQHandler(void) {  // This IRQ is triggered when USART1 receives the next symbol
    ...
    if (isOverflow) {
        ...
        rxPos = 0;
        LL_GPIO_ResetOutputPin(GPIOG, LL_GPIO_PIN_14);  // Turn off UART LED
...
void EXTI0_IRQHandler(void) {
    ...
    EXTI->PR |= EXTI_PR_PR0;    // Clear with HIGH level
